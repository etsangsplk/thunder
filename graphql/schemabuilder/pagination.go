...
func getConnection(key string, nodes []interface{}, args ConnectionArgs) (Connection, error) {
	...
	for _, val := range nodes {
		...
		edges = append(edges, Edge{Node: val, Cursor: base64.StdEncoding.EncodeToString(keyString)})
	}
	...
	pageInfo := PageInfo{HasNextPage: nextPage, EndCursor: endCursor, StartCursor: startCursor, HasPrevPage: prevPage}
...
func (sb *schemaBuilder) buildPaginatedField(typ reflect.Type, f interface{}) (*graphql.Field, error) {
	...
	if funcCtx.funcType.Out(0).Kind() != reflect.Slice {
		return nil, fmt.Errorf("paginated field func must be passed a function which returns a slice type")
	}
